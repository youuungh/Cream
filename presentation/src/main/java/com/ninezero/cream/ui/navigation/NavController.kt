    ...
    @Composable
    fun rememberCreamNavController(
        navController: NavHostController = rememberNavController()
    ): CreamNavController = remember(navController) {
        CreamNavController(navController)
    }

    @Stable
    class CreamNavController(val navController: NavHostController) {

        fun navigateBack() { navController.navigateUp() }

        fun navigateToCart() { navController.navigate(AppRoutes.CART) }

        fun navigateToSearch() { navController.navigate(AppRoutes.SEARCH) }
    ...
    fun NavGraphBuilder.addMainGraph(
        ...
        composable(
            ...
            deepLinks = listOf(navDeepLink { uriPattern = "${AppRoutes.DEEP_LINK_SCHEME}${AppRoutes.MAIN_HOME}" })
        ) {
            HomeScreen(
        ...
        composable(
            ...
            deepLinks = listOf(navDeepLink { uriPattern = "${AppRoutes.DEEP_LINK_SCHEME}${AppRoutes.MAIN_CATEGORY}" })
        ) {
            CategoryScreen(
                onCategoryClick = { categoryId, categoryName -> onCategoryClick(categoryId, categoryName, it) },
                onCartClick = onCartClick,
                modifier = modifier
            )
        }
